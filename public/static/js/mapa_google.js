...
window.app = Vue.createApp({
    ...
    methods: {
        ...
        crearSvgBadgePrioridadAlta() {
            return `<g transform="translate(25, 10)">
                <g>
                    <circle cx="0" cy="0" r="7" fill="#DC3545" stroke="#FFFFFF" stroke-width="1.5"/>
                    <text x="0" y="3.5" text-anchor="middle" font-family="Arial, sans-serif" font-size="10" font-weight="bold" fill="#FFFFFF">!</text>
                    <animateTransform attributeName="transform" type="translate" values="0,0; 0,-3; 0,0" dur="0.75s" repeatCount="indefinite"/>
                </g>
            </g>`;
        },

        crearIconoGoogleMaps(color, contenidoCentro, esNumero = false, prioridadAlta = false) {
            const fontFamily = esNumero
                ? 'Open Sans, Segoe UI, sans-serif'
                : 'Apple Color Emoji, Segoe UI Emoji, Noto Color Emoji, sans-serif';
            const fontWeight = esNumero ? 'font-weight="700"' : '';
            const pinCore = `<path d="M16 2.5C10.75 2.5 6.5 6.75 6.5 12c0 7.1 9.5 17.5 9.5 17.5S25.5 19.1 25.5 12C25.5 6.75 21.25 2.5 16 2.5Z" fill="${color}" stroke="#FFFFFF" stroke-width="2"/>
            ...
            const viewHeight = 39;
            const displayH = 39;
            const anchorY = Math.round(((29.5 - viewTop) / viewHeight) * displayH);

            return {
                url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`
                    <svg width="32" height="${displayH}" viewBox="0 ${viewTop} 32 ${viewHeight}" xmlns="http://www.w3.org/2000/svg" overflow="visible">
                        ${pinCore}
                        ${this.crearSvgBadgePrioridadAlta()}
                    </svg>
                `)}`,
                size: new google.maps.Size(32, displayH),
        ...
        vincularEventosPopupGoogle(marker, reclamo) {
            ...
            if (marker._grupoId) {
                ...
                if (btnPrev) {
                    btnPrev.onclick = (event) => {
                        ...
                        this.navegarPopupGrupoGoogle(marker, -1);
                    };
                }
        ...
        navegarPopupGrupoGoogle(marker, delta) {
            ...
            this.abrirPopupMarcadorGoogle(marker, nuevoIndice);
        },

        abrirPopupMarcadorGoogle(marker, indice = null) {
            ...
            setTimeout(() => this.vincularEventosPopupGoogle(marker, reclamo), 100);
        },

        actualizarIconoMarcadorGoogle(marker) {
            ...
            marker.setIcon(this.crearIconoGoogleMaps(color, contenido, esNumero, prioridadAlta));
        ...
        crearMarcadorReclamoGoogle(reclamo, coordenadas, grupo = null) {
            const color = this.colorEstadoReclamo(reclamo.municipalidad_estado || 'Recibido');
            ...
            const marker = new google.maps.Marker({
                ...
                map: this.map,
                title: esGrupo
                    ? `${cantidadVisible} reclamos en este domicilio`
                    : `Reclamo #${reclamo.municipalidad_id}`,
                optimized: false,
                icon: this.crearIconoGoogleMaps(
                    this.colorEstadoReclamo(reclamoRef.municipalidad_estado || 'Recibido'),
                    contenido,
                    esNumero,
                    prioridadAlta
                )
            ...
            marker.addListener('click', () => {
                this.abrirPopupMarcadorGoogle(marker, 0);
            });
        ...
        async agregarMarcadoresReclamos() {
            ...
            for (const grupo of grupos) {
                ...
                if (grupo.reclamos.length > 1) {
                    this.crearMarcadorReclamoGoogle(grupo.reclamos[0], grupo.coordenadas, {
                        reclamos: grupo.reclamos,
                        grupoId: `grupo-google-${++contadorGrupos}`
                    });
                } else {
                    this.crearMarcadorReclamoGoogle(grupo.reclamos[0], grupo.coordenadas);
                }
        ...
        async eliminarUbicacionPersonalizada() {
            ...
            const mensajeConfirmacion = '¿Está seguro de que desea eliminar la ubicación personalizada? El punto volverá a la ubicación automática según la dirección del reclamo (Google Maps).';
        ...
        aplicarFiltroMarcadores() {
            this.marcadores.forEach((marker) => {
                if (marker._reclamosGrupo) {
                    const visibles = this.obtenerReclamosVisiblesGrupo(marker._reclamosGrupo);
                    marker.setVisible(visibles.length > 0);
                    if (visibles.length > 0) {
                        this.actualizarIconoMarcadorGoogle(marker);
                    }
                    return;
                }
